...
Examination of documented adversarial incidents reveals patterns distinct from academic attack formulations. These cases illustrate the sociotechnical nature of ML security threats and motivate bridging theory and practice.

\begin{itemize}[noitemsep]
    \item \textbf{EchoLeak (2025).} A malicious image embedded in a phishing email caused Microsoft's GitHub Copilot to leak proprietary code~\citep{EchoLeak2025}. The image induced Copilot to follow a link and parse remote content, exposing internal data. This supply-chain attack exploited input handling rather than gradient-based perturbation.
    
    \item \textbf{Bard document exploit.} A vulnerability in Google's Bard, disclosed through bug bounty, demonstrated that hidden prompts in shared Google Docs could be executed by Bard's document summarization feature~\citep{HackerOneBard}. Encoded instructions bypassed normal input channels to induce information leakage.
    
    \item \textbf{AgentFlayer (2025).} This demonstration~\citep{AgentFlayer2025} showed that a single poisoned document, when processed by an LLM-based agent with browsing and code execution capabilities, could reveal API keys. The attack chained multiple steps by exploiting the agent's trust in retrieved content.
    
    \item \textbf{Training data extraction.} Nasr et al.~\citep{Nasr2023} presented a divergence attack on ChatGPT that extracted training data more effectively than prior membership inference approaches, demonstrating privacy risks in deployed systems.
    
    \item \textbf{Model extraction at scale.} Liu et al.~\citep{Liu2024} demonstrated high-fidelity model extraction through black-box APIs when query costs are low, highlighting economic dimensions of ML security.
\end{itemize}

These incidents share common themes: attackers exploit system integrations, content handling, and human factors rather than computing optimal perturbations. They underscore the importance of considering the full deployment context, including input sanitization, rate limiting, and user education, alongside model-centric robustness.
