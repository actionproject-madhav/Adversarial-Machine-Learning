...
\textbf{Reliability:} All 71 papers are independently coded by two authors. Inter-rater reliability is measured using Cohen's $\kappa$ for categorical variables (G1--G6, T1--T2, Q1), targeting $\kappa \geq 0.80$ (substantial agreement). Disagreements are resolved through discussion to produce final consensus codes. Adoption dates are verified via \texttt{git log} with string-search (\texttt{-S}) to ensure accuracy.
